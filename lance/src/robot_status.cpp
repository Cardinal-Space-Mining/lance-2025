...
public:
    RobotStatusServer() :
        ...
        robot_state_service
        {
            this->create_service<SetRobotMode>(
...
protected:
    ...
    {
        return (this->robot_mode > 0 ?
            (duration_cast<milliseconds>(WATCHDOG_TELEOP_FEED_TIME).count()) :
            (this->robot_mode < 0 ?
                -duration_cast<milliseconds>(WATCHDOG_AUTO_FEED_TIME).count() : 0 ) );
    }

protected:
    rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr
        watchdog_status_pub;
    rclcpp::Service<SetRobotMode>::SharedPtr
        robot_state_service;
