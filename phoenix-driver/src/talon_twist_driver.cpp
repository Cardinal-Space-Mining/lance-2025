...
class TwistToTalon : public rclcpp::Node {
    public:
        TwistToTalon() : Node("twistToTalon")
            , drive_right_pub(this->create_publisher<TalonCtrl>(
                    "track_right_ctrl_auto", 10))
            , drive_left_pub(this->create_publisher<TalonCtrl>(
                    "track_left_ctrl_auto", 10))
            , drive_sub(this->create_subscription<geometry_msgs::msg::Twist>(
                    "drive_twist", 10, [this](const geometry_msgs::msg::Twist& msg) {
                        this->translate(msg);
                    })) {}
