...
#define INIT_TALON_PUB_SUB(device)                         \
    device##_pub_sub                                       \
    {                                                      \
        this->create_publisher<TalonInfoMsg>(                 \
            ROBOT_TOPIC(#device "/info"),                  \
            rclcpp::SensorDataQoS{}),                      \
        this->create_publisher<TalonFaultsMsg>(               \
            ROBOT_TOPIC(#device "/faults"),                \
            rclcpp::SensorDataQoS{}),                      \
        this->create_subscription<TalonCtrlMsg>(              \
...
{
    this->op_pub->publish(StringMsg{}.set__data("build motor configs"));
...
{
    this->op_pub->publish(StringMsg{}.set__data("init serial"));
...
void Phoenix6Driver::configure_motors_cb(units::time::second_t timeout)
{
    ...
    auto s1 = trencher.GetConfigurator().Apply(this->trencher_config, timeout);
...
void Phoenix6Driver::execute_ctrl_cb(TalonFX& motor, const TalonCtrlMsg& msg)
{
    this->op_pub->publish(StringMsg{}.set__data("exec control"));
    // std::cout << "begin exec motor ctrl -- ";
    if (this->is_disabled)
    {
    ...
    // std::cout << "end exec motor ctrl" << std::endl;
}
...
{
    c_Phoenix_Diagnostics_Create_On_Port(1250);
